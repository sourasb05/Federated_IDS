# server_GMM.py
#
# FedAvg server using ClientGMM clients.
# Structure is identical to server_RVAE.py — only client class,
# model save directory, and results filename differ.
#
# The server never sees, shares, or aggregates GMM generators.
# Generators live entirely on each client — private per client.

import os
import torch
from client_GMM import ClientGMM
from utils import save_results_as_json


def server_gmm(args, model, device, domains_path,
               client_distributions, max_client_participants,
               tot_time_steps, project_root,
               n_components: int = 10):
    """
    Executes FedAvg server logic with ClientGMM clients.
    Each client privately maintains one BayesianGMM generator per class
    per domain and replays past domains during local training.

    Parameters
    ----------
    n_components : max GMM components per class (BayesianGMM prunes automatically)
    """
    print("\n--- Starting FedAvg with GMM Generative Replay ---")

    models_dir = os.path.join(project_root, 'saved_models_gmm')
    ...
    print(f"\nInitialized {len(client_list)} ClientGMM clients.")
