# server_TVAE.py
#
# Identical to server.py (FedAvg) in all aggregation logic.
# The ONLY differences:
#   - Uses ClientTVAE instead of Client
#     (generative replay is fully encapsulated in ClientTVAE)
#   - Saves results to 'tvae_metrics.json' instead of 'fedavg_metrics.json'
#
...
import os
import torch
from client_TVAE import ClientTVAE
from utils import save_results_as_json


def server_tvae(args, model, device, domains_path,
                client_distributions, max_client_participants,
                tot_time_steps, project_root):
    """
    Executes FedAvg server logic with ClientTVAE clients.
    Each client privately maintains one frozen TVAE generator
    per domain and replays past domains during local training.
    """
    print("\n--- Starting FedAvg with TVAE Generative Replay ---")

    models_dir = os.path.join(project_root, 'saved_models_tvae')
    os.makedirs(models_dir, exist_ok=True)

    # ── initialise clients ───────────────────────
    # ClientTVAE has identical __init__ signature to Client
    client_list = []
    ...
    print(f"\nInitialized {len(client_list)} ClientTVAE clients.")
    ...
    # ════════════════════════════════════════════════════════
    # MAIN LOOP — identical to server.py
    # ════════════════════════════════════════════════════════
    for t in range(tot_time_steps):
        ...
        if len(active_clients) > 0:
            for iter in range(args.global_iters):
                ...
                # ── local training ────────────────
                # ClientTVAE.train() handles:
                #   1. train TVAE generator on current domain
                #   2. build replay loader from past generators
                #   3. train LSTM on real + replay data
        ...
        # load best model from current time step
        ...
        # evaluate on all past + current time steps
    ...
    #   generator_quality         — original feature space (T*F dims)
    #   generator_quality_pca     — PCA space (pca_dim dims, no inverse-PCA smoothing)
    generator_quality     = {}
    generator_quality_pca = {}
    for c in client_list:
        for key, eval_result in c.generator_eval_results.items():
            client_key = f'client_{c.client_id}_{key}'
            if key.endswith('_pca_space'):
                generator_quality_pca[client_key] = eval_result['quality']
            else:
                generator_quality[client_key] = eval_result['quality']

    results_to_save['generator_quality']     = generator_quality
    results_to_save['generator_quality_pca'] = generator_quality_pca
