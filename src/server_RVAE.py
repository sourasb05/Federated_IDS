# server_RVAE.py
#
# FedAvg server using ClientRVAE clients.
# Aggregation logic is identical to server_TVAE.py.
#
# The ONLY differences from server_TVAE.py:
#   - Uses ClientRVAE instead of ClientTVAE
#   - Saves results to 'rvae_metrics.json'
#   - Reads generator_quality from rvae_plots (via generator_eval_results)
#
...
import os
import torch
from client_RVAE import ClientRVAE
from utils import save_results_as_json


def server_rvae(args, model, device, domains_path,
                client_distributions, max_client_participants,
                tot_time_steps, project_root):
    """
    Executes FedAvg server logic with ClientRVAE clients.
    Each client privately maintains one frozen RVAE generator
    per domain and replays past domains during local training.
    """
    print("\n--- Starting FedAvg with RVAE Generative Replay ---")

    models_dir = os.path.join(project_root, 'saved_models_rvae')
    ...
    print(f"\nInitialized {len(client_list)} ClientRVAE clients.")
    ...
    # collect generator quality from all clients
    generator_quality = {}
    for c in client_list:
        for domain_key, eval_result in c.generator_eval_results.items():
            generator_quality[f'client_{c.client_id}_{domain_key}'] = \
                eval_result['quality']
