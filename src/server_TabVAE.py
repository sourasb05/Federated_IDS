...
import torch
from client_TabVAE import ClientTabVAE
from utils import save_results_as_json

def server_tabvae(args, model, device, domains_path,
                client_distributions,
                max_client_participants,
    ...
    for i in range(0, max_client_participants):
        c = ClientTabVAE(client_id=i, args=args, domain_path=domains_path, assigned_domains=client_distributions[i], device=device, model=model)
    ...
    for t in range(tot_time_steps):
        ...
        if t > 0:
            ...
            prev_model_path = os.path.join(models_dir, f"best_global_model_ts_{t}.pth")
        ...
        current_best_model_path = os.path.join(models_dir, f"best_global_model_ts_{t+1}.pth")
    ...
    save_results_as_json('TabVAE_fedavg_metrics.json', results_to_save, project_root, results_folder)
